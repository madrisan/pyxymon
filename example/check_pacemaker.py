...
def cluster_resources():
    ...
    def _pack_data(resource):
        ...
        node = list(node.getAttribute("name") for node in nodes if nodes.length > 0)
...
def cluster_resource_groups():
    ...
    return dict(
        (
            e.getAttribute("id"),
            list(e.getAttribute("id") for e in e.getElementsByTagName("primitive")),
        )
        for e in elements
    )
...
def check_resource_groups_status(
    ...
    groups = list(
        ...
        if resource in resource_groups[group]
    )
...
def get_cluster_infos(resource_groups_cfg_map):
    ...
    _get_local_node_attr = lambda attr: cl_local_node_status.get(attr, "Unknown")
    ...
    node_resources = dict(
        (k, v) for k, v in all_resources.items() if v.get("node") == node_name
    )
    ...
    resource_group_required = resource_groups_cfg_map.get(node_name, None)
...
def check_cluster_status(test_name, resource_groups_cfg_map, check_daemons):
    ...
    node_status = (
        ...
        if resources_match_cfg
        else "{0} (resources have switched)".format(cluster_infos["node_status"])
    )
    # message - title
    xymon.title('Pacemaker cluster "{0}"'.format(cluster_name))

    # message - cluster node status
    node_color = pymon.STATUS_OK if node_status == "online" else pymon.STATUS_CRITICAL
    xymon.section(
        "Node Status", "{0} - {1} {2}".format(node_name, node_status, node_color)
    )
    ...
    # message - resources
    resources_summary = "{c[resources_running]} resources running:\n\n".format(
        c=cluster_infos
    )
    ...
    resources_list_with_status = "\n".join(
        " {0} {1:<22} {2}".format(resources_color(v), k, v.get("resource_agent"))
        for k, v in cluster_infos["node_resources"].items()
    )
    xymon.section(
        "Cluster Resources",
        "{0}{1}".format(resources_summary, resources_list_with_status),
    )
    if not all(
        [
            item.split()[0] == pymon.STATUS_OK
    ...
    if check_daemons:
        ...
        if not all(status.is_service_running(service) for service in cluster_services):
            xymon.color = pymon.STATUS_CRITICAL
...
if __name__ == "__main__":
    ...
    try:
        opts, args = getopt.getopt(
            sys.argv[1:], "dr:t:h", ["daemons", "resource=", "test=", "help"]
        )
